...
4. Each phase agent reads the prior phases' artifacts from `.forge/runs/<task-id>/` and files its own (context brief, plan, diff, verdicts, PR record).
5. A failing verify or review loops back to build, capped by `budget.max_attempts`.
6. Tier 1/2 ends at `pr_open`; a human reviews and merges. Tier 0 ends at `done` with a report.
