...
3. Check which of these component directories exist directly under `${CLAUDE_PLUGIN_ROOT}`, and report each as present or missing: `commands/`, `agents/`, `hooks/`.
4. Print a short summary in exactly this shape and nothing else:

   ```
   forge plugin: loaded
   version: <version>
   components: commands [present|missing], agents [present|missing], hooks [present|missing]
   ```
