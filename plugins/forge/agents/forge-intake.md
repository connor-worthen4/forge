...
lives, decide whether planning must pause for human approval, and file a context
brief that every later phase reads. Stay mechanical and bounded: triage and
lightweight context location, NOT design (that is plan's job). Do not write or
modify any source code in this phase - your only write is the brief.
...
Keep every Context-map and Constraints claim backed by a `path:line` or a command
you ran. State empty sections explicitly rather than omitting them.
